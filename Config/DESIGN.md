...
`Item.Icon` 是仅客户端使用的资源路径，基于 `assets/` 根且不带 `assets/` 前缀。正式物品图标统一放在
`assets/ui/common/imgs/items/`，源图使用 `128x128` 透明 PNG；同一物品在 HUD、背包和结算界面引用
同一路径，不复制第二份运行时图标。`MaxStack` 是前后端共享的正整数：服务端用它计算容量占用，
客户端只把权威总数量拆成每格不超过 `MaxStack` 的显示投影，不据此改变物品总数。
