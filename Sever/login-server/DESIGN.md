...
- `WECHAT_APP_ID` 与 `WECHAT_APP_SECRET` 只从服务端环境读取。缺少任一变量时只禁用真实微信请求并返回稳定的 `WECHAT_CONFIG_MISSING`，不得阻止 Login Server 启动或影响其他认证适配器；Test/Production 部署验收必须在对外开放前确认配置完整。微信认证适配器的固定开发 code 仅供直接专项联调，要求 Local/Test 进程显式设置 `WECHAT_DEVELOPER_CODE_ENABLED=true`；小游戏客户端强制账号模式改走 `GUEST` 开发凭据，不依赖该开关且不构成微信身份。Production 必须关闭该变量。
