...
- 对象池生命周期回归用例及公共测试夹具按主题放在 `assets/testAndSample/testXxx/`，场景、脚本和用例均留在主题包内。直接挂载 `.ls` 的入口可为 IDE 兼容保留 Runtime 加载；正式调用式入口必须使用 `@IEditor.menu` → `Editor.scene.runScript` → `@IEditorEnv.regClass`，依赖 LayaAir 3.3 的编辑器脚本隔离，且不得被正式入口、正式场景或正式资源引用。正式微信构建产物必须搜索确认测试代码和资源均不存在。用例可创建无显示测试实体，但必须驱动真实 `BaseScene.update()`，不能用独立计时器模拟对象删除顺序。任何碰撞候选在解析和命中前都通过 `getLiveObject(uid)` 取得，不保留实体对象引用。
