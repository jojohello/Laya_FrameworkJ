...
- AI 决定目标、技能和进入攻击距离后是否停止；Run 状态执行逐帧靠近，同队绕行是 Run 调用的场景位移策略。`CharacterSceneObj` 每帧只提交一次最终位移，普通 `BaseScene` 原样通过期望位移，`BattleScene` 才通过局部避让器查询前进方向上的同队候选并返回局部修正。避让器只保存 UID 和稳定侧向，不得后置推挤已移动对象、持有实体引用或执行全局两两扫描。绕行侧向是相对实际前排阻挡者的切线，UID 对确定顺/逆时针选择；任何避让强度都必须保留正的朝目标前进分量，不能在近距离改为后退。AI 的攻击判断使用 `中心距离 <= Skill.CastRange + target.range + 浮点容差`；传给 Run 的停止距离使用 `max(0, Skill.CastRange + target.range - 5)`。5 个世界单位是移动到达余量，不改变技能合法范围。本帧足以进入停止范围时，角色直接落在停止边界，绕行器不得侧向改写最后一段到达位移。
- `BaseSceneObj.range` 是唯一逻辑圆半径，不再额外维护 `radius`。空间切分占位、技能/子弹碰撞和局部避让都读取 `range`；角色值来自 `Character.range`。显示缩放和素材透明边界不得隐式改变该半径。
...
- 空间切分默认不接纳实体；只有可作为技能或子弹命中候选的实体才覆盖 `canEnterSpatialIndex()` 显式加入。当前常规候选是 `CreatureSceneObj`；子弹、特效和纯显示实体只发起或承载查询，不进入候选索引。未来吸收/拦截子弹的实体必须单独声明此能力和筛选语义。
...
- 子弹和技能碰撞必须通过 `BaseScene` 的队伍空间切分查询候选，不得回退到全对象表扫描。子弹只作为查询发起者，不进入队伍空间索引，也不能成为其他子弹的命中目标；命中筛选必须排除 `SceneObjType.Bullet`。目标型子弹在延迟执行时若目标已不再 live，直接取消，不得回退为查询施法者队伍。
- 子弹飞行图集是纯表现资源：`Bullet.Resource` 指向同名 PNG 的 `.atlas`，空值保留圆点兜底。标准帧键为 `frame_00.png` 起的连续序列，默认按全部六帧循环（90ms/帧、50px 显示）。子弹不创建角色式动作范围/时长配置，因为当前只有一个固定循环动作；新增多动作子弹时，先确认是否真的需要独立动作语义，再扩展接口或配置。视觉尺寸不得隐式改变 `Range` 命中半径。
- 有方向语义的子弹图集以“向右飞行”为默认源图朝向；直线 Movement 在起飞时用 `pointTo()` 设置朝向，追踪 Movement 用 `turnToDirection()` 更新朝向。表现节点必须作为子弹模型的子节点继承旋转，不得单独镜像或反向旋转，否则火尾等后向细节会与运动方向脱节。
- 图集异步完成时必须确认子弹仍处于同一生命周期；回池和销毁时停止动画、移除显示子节点并归还图集引用，不能让迟到加载结果重新挂到旧子弹。
- 对象池生命周期回归用例及公共测试夹具按主题放在 `assets/testAndSample/testXxx/`，场景、脚本和用例均留在主题包内。测试脚本集必须关闭 Runtime 发布；正式调用式入口使用 `@IEditor.menu` → `Editor.scene.runScript` → `@IEditorEnv.regClass`，依赖 LayaAir 3.3 的编辑器脚本隔离，且不得被正式入口、正式场景或正式资源引用。直接挂载 `.ls` 只保留源资源，不作为正式运行入口。正式微信构建产物必须搜索确认测试代码和资源均不存在。用例可创建无显示测试实体，但必须驱动真实 `BaseScene.update()`，不能用独立计时器模拟对象删除顺序。任何碰撞候选在解析和命中前都通过 `getLiveObject(uid)` 取得，不保留实体对象引用。

## 战斗表现特效

- `EffectSceneObj` 只承载表现：使用 `ResFrameAnimation` 播放标准 PNG + atlas 的一次性帧动画，按场景逻辑时间结束并随 SceneObj 对象池回收；不得建立独立 Timer，也不得进入空间切分。
- 伤害和有效恢复在结算的同一调用栈内，先快照场景、队伍和命中坐标，再创建受击或恢复特效。异步图集加载完成时只检查 Effect 自身的生命周期；不得保存或重新查询已经死亡、回池或销毁的 target/caster。
- 战斗入口预加载正式受击与恢复图集；Effect 回收时必须停止动画、移除显示节点并通过 `ResourceMgr.recoverRes()` 归还图集实例。
- 角色战斗特效的挂点由 `Character.centerOffsetY` 配置，语义是从逻辑脚底点到中心点的 Y 位移（负数向上）。受击和恢复统一通过 `getCombatEffectCenterY()` 取快照，不得在表现对象中反向绑定角色模型。
- 命中特效由 `Damage` Action 的可选第二参数 `CombatEffect.ID` 决定，省略时使用默认受击特效，`0` 表示明确关闭。特效资源路径、时长、缩放和帧数只在 `CombatEffect.csv` 配置，不将不同技能的表现分支硬编码进伤害或子弹逻辑。
- 正常创建、释放、回收、治疗和预期的 live-target 取消不记录日志；配置或生命周期异常使用所属模块的稳定前缀输出。
